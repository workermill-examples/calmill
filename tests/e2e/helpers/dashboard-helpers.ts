...
export function getStatCards(page: Page): Locator {
  return page.locator('[class*="grid"] a[href="/bookings"], [class*="grid"] a[href="/event-types"]');
}
...
export function getStatCardByLabel(page: Page, label: string): Locator {
  return page.locator("text=" + label).locator("..").locator("..");
}
...
export async function createEventType(
  ...
  // Fill title
  await page.locator('input[name="title"], input[placeholder*="title" i], input[placeholder*="name" i]').first().fill(title);
...
export function getBookingCardByAttendee(page: Page, attendeeName: string): Locator {
  return page.locator("text=" + attendeeName).locator("..").locator("..");
}
...
export async function searchBookings(page: Page, query: string): Promise<void> {
  const searchInput = page.locator('input[placeholder*="search" i], input[placeholder*="attendee" i]');
...
export async function applyBookingFilters(page: Page, eventTypeTitle?: string): Promise<void> {
  if (eventTypeTitle) {
    const eventTypeFilter = page.locator('select[name="eventType"], select[aria-label*="event type" i]');
...
export function selectTimezone(page: Page): Locator {
  return page.locator('select[aria-label*="timezone" i], select[name="timezone"]').first()
    .or(page.locator('select').filter({ hasText: /America|Europe|Asia|UTC/ }).first());
}
