...
test.describe("Dashboard Flows", () => {
  ...
  test.describe("Event types list", () => {
    ...
    test("event types list shows existing event types from seed", async ({ page }) => {
      ...
      const cards = page.locator('[class*="rounded-lg"][class*="border"]').filter({ hasNot: page.locator('nav') });
  ...
  test.describe("Event type editor", () => {
    ...
    test("event type editor has all five tabs", async ({ page }) => {
      ...
      await expect(page.locator("button", { hasText: "Limits" }).or(page.locator("button", { hasText: "Limits & Buffers" }))).toBeVisible();
    ...
    test("clicking Limits tab shows limits content", async ({ page }) => {
      const limitsTabName = (await page.locator("button", { hasText: "Limits & Buffers" }).count()) > 0
        ? "Limits & Buffers"
        : "Limits";
      ...
      await expect(
        page.locator("text=notice").or(page.locator("text=buffer")).or(page.locator("text=Notice")).first()
      ).toBeVisible({ timeout: 5_000 });
    });
  });
  ...
  test.describe("Event type delete flow", () => {
    ...
    test.beforeEach(async ({ page }) => {
      ...
      await page.locator('[role="dialog"] button[type="submit"], [role="dialog"] button', { hasText: /^Create/ }).first().click();
  ...
  test.describe("Bookings list tabs", () => {
    ...
    test("bookings list shows empty state or booking cards", async ({ page }) => {
      ...
      const emptyState = page.locator("text=No bookings").or(page.locator("text=no bookings").or(page.locator("text=empty")));
      const bookingCards = page.locator('[class*="rounded-lg"][class*="border"]').filter({ hasNot: page.locator("nav") });
  ...
  test.describe("Booking status actions", () => {
    ...
    test("clicking Accept on a pending booking updates its status", async ({ page }) => {
      ...
      if (await acceptButton.count() > 0) {
        ...
        test.info().annotations.push({ type: "note", description: "No pending bookings available to test Accept action" });
      }
    });
  ...
  test.describe("Availability schedule editor", () => {
    ...
    test("availability page shows weekly grid with day names", async ({ page }) => {
      ...
      await expect(page.locator("text=Monday").or(page.locator("text=Mon")).first()).toBeVisible({ timeout: 10_000 });
    });
    ...
    test("clicking Save shows saved confirmation", async ({ page }) => {
      ...
      await expect(
        page.locator("text=Saved").or(page.locator("text=saved")).first()
      ).toBeVisible({ timeout: 10_000 });
    });
  });
  ...
  test.describe("Profile settings", () => {
    ...
    test("settings page shows Preferences section", async ({ page }) => {
      await expect(
        page.locator("text=Preferences").or(page.locator("h2", { hasText: "Preferences" })).first()
      ).toBeVisible({ timeout: 10_000 });
    });
