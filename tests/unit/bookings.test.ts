...
import {
  GET as _getBooking,
  PATCH as _updateBookingStatus,
  PUT as _rescheduleBooking,
} from '@/app/api/bookings/[uid]/route';
...
describe('PATCH /api/bookings/[uid]', () => {
  ...
  it('accepts a PENDING booking (host action)', async () => {
    const [request, context] = makeUidRequest('booking-uid-abc123', { action: 'accept' }, 'PATCH');
  ...
  it('returns 404 when booking is not found', async () => {
    ...
    const [request, context] = makeUidRequest('non-existent-uid', { action: 'cancel' }, 'PATCH');
    const response = await updateBookingStatus(request, context);
