...
test.describe("Event Types", () => {
  ...
  test.describe("Authenticated Event Types", () => {
    authTest("loads event types page with correct content", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      ...
      await expect(page.locator("text=Manage your schedulable meeting types")).toBeVisible();

      // Check for "New Event Type" button
      await expect(page.locator('text="New Event Type"')).toBeVisible();
    });

    authTest("displays seeded event types from seed data", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      ...
      const eventTypeCards = page.locator("[data-testid='event-type-card'], .card").filter({
    ...
    authTest("shows event type details correctly", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(2000);
      ...
      await expect(firstCard.locator('button:has-text("Active"), button:has-text("Inactive")')).toBeVisible();

      // Should show action buttons
      await expect(firstCard.locator('button:has-text("Edit")')).toBeVisible();
      await expect(firstCard.locator('button[aria-label="More actions"]')).toBeVisible();
    });

    authTest("can create new event type", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      ...
      await page.fill('textarea[name="description"]', "This is a test event type created via E2E test");
    ...
    authTest("can edit existing event type", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
    ...
    authTest("can toggle event type active/inactive", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);

      // Find first event type and its current state
      const firstCard = page.locator(".card").first();
      const toggleButton = firstCard.locator('button:has-text("Active"), button:has-text("Inactive")');
    ...
    authTest("can duplicate event type", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
      ...
      const firstMoreButton = page.locator('button[aria-label="More actions"]').first();
    ...
    authTest("can delete event type", async ({ authenticatedPage: page }) => {
      ...
      const moreButton = testEventCard.locator('button[aria-label="More actions"]');
    ...
    authTest("can copy event type link", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);

      // Click copy link button for first event type
      const firstCopyButton = page.locator('button:has-text("Copy Link")').first();
    ...
    authTest("search functionality works", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
      ...
      await page.fill('input[placeholder="Search event types..."]', "Quick Chat");
      ...
      const hasQuickChat = await page.locator('text="Quick Chat"').isVisible();
      if (hasQuickChat) {
    ...
    authTest("show inactive toggle works", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
      ...
      const hasInactiveSection = await page.locator('text="Inactive Event Types"').isVisible();
      // This might not be visible if there are no inactive event types, which is fine

      // Toggle should be pressed
      await expect(page.locator('button[aria-label="Show inactive event types"][aria-pressed="true"]')).toBeVisible();
    });

    authTest("displays public URLs correctly", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
  ...
  test.describe("Error Handling", () => {
    authTest("handles API errors gracefully", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator('text="Failed to load event types"')).toBeVisible();
  ...
  test.describe("Responsive Design", () => {
    authTest("event types page is responsive on mobile", async ({ authenticatedPage: page }) => {
      // Set mobile viewport
      ...
      await expect(page.locator('button:has-text("Copy Link")')).not.toBeVisible();
    ...
    authTest("event types page is responsive on tablet", async ({ authenticatedPage: page }) => {
      // Set tablet viewport
  ...
  test.describe("Accessibility", () => {
    authTest("event types page has proper heading structure", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
      ...
      await expect(page.locator('h2:has-text("Active Event Types")')).toBeVisible();
    });

    authTest("interactive elements are keyboard accessible", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
      ...
      const searchInput = page.locator('input[placeholder="Search event types..."]');
    ...
    authTest("toggle buttons have proper aria labels", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);

      // Show inactive toggle should have aria label
      const showInactiveToggle = page.locator('button[aria-label="Show inactive event types"]');
      await expect(showInactiveToggle).toBeVisible();

      // Event type toggles should have descriptive aria labels
      const firstEventToggle = page.locator('button[aria-label*="Activate"], button[aria-label*="Deactivate"]').first();
      await expect(firstEventToggle).toBeVisible();
    });

    authTest("dropdown menus are keyboard navigable", async ({ authenticatedPage: page }) => {
      await page.goto("/event-types");
      await page.waitForTimeout(1000);
