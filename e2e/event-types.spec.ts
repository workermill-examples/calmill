...
test.describe("Event Types Management", () => {
  ...
  test.describe("Event types list", () => {
    ...
    test("each event type card has a Delete button", async ({ page }) => {
      const deleteButton = page.locator(
        'button[aria-label="Delete 30 Minute Meeting"]'
      );
  ...
  test.describe("Event type creation", () => {
    ...
    test("creating an event type navigates to editor", async ({ page }) => {
      ...
      const url = page.url();
      const isOnEditorOrList =
        url.includes("/event-types") || url.match(/\/event-types\/[^/]+/);
  ...
  test.describe("Event type editor", () => {
    ...
    test("clicking Availability tab shows schedule content", async ({
      page,
    }) => {
    ...
    test("clicking Limits tab shows notice/buffer content", async ({
      page,
    }) => {
      const limitsTabName =
        (await page
    ...
    test("clicking Booking Form tab shows form fields content", async ({
      page,
    }) => {
    ...
    test("clicking Recurring tab shows recurring options", async ({ page }) => {
      ...
      await expect(
        page
          .locator("text=recurring")
          .or(page.locator("text=Recurring"))
          .first()
      ).toBeVisible({ timeout: 5_000 });
    });
  });
  ...
  test.describe("Event type active toggle", () => {
    ...
    test("inactive event types show inactive state in toggle", async ({
      page,
    }) => {
      // The Coffee Chat event type is created as inactive in seed data
      // Its toggle should have aria-checked="false"
      const coffeeToggle = page.locator(
        'button[role="switch"][aria-label*="Coffee Chat"]'
      );
  ...
  test.describe("Custom questions in Booking Form tab", () => {
    ...
    test("Booking Form tab shows default attendee fields", async ({ page }) => {
    ...
    test("Add Question button is visible on Booking Form tab", async ({
      page,
    }) => {
  ...
  test.describe("Public event type URL", () => {
    test("visiting the public URL for 30min shows the booking page", async ({
      page,
    }) => {
      await page.goto("/demo/30min");

      await expect(
        page.locator("h1", { hasText: "30 Minute Meeting" })
  ...
  test.describe("Inactive event types visibility", () => {
    test("inactive event types are hidden from public profile", async ({
      page,
    }) => {
      ...
      const coffeeChatCard = page.locator(
        'a[aria-label^="Book Coffee Chat"]'
      );
