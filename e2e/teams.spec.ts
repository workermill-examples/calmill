...
test.describe("Teams Management", () => {
  test.describe("Teams List Page", () => {
    authenticatedTest("loads teams page and displays existing teams", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator("text=Manage your teams and collaborate with others")).toBeVisible();
      ...
      try {
        ...
        const hasTeamContent = await page.locator('text="members"').first().isVisible({ timeout: 2000 }).catch(() => false);
        const hasEmptyState = await page.locator('text="No teams yet"').isVisible({ timeout: 2000 }).catch(() => false);

        expect(hasTeamContent || hasEmptyState).toBe(true);
      }
    });

    authenticatedTest("can create a new team", async ({ authenticatedPage: page }) => {
      await page.goto("/teams");
      ...
      await page.fill('input[id="team-logo"]', "https://example.com/logo.png");
      await page.fill('textarea[id="team-bio"]', "This is a test team for E2E testing");
    ...
    authenticatedTest("validates team creation form", async ({ authenticatedPage: page }) => {
      await page.goto("/teams");
      ...
      await expect(page.locator('input[id="team-slug"]')).toHaveValue(/test-team/);
    });

    authenticatedTest("shows error for duplicate team slug", async ({ authenticatedPage: page }) => {
      await page.goto("/teams");
      ...
      await expect(page.locator('text="A team with this slug already exists"')).toBeVisible();
    });
  });

  test.describe("Team Management Page", () => {
    authenticatedTest("can navigate to team management from list", async ({ authenticatedPage: page }) => {
      await page.goto("/teams");

      // Look for existing team or create one first
      const existingTeamLink = page.locator('a[href*="/teams/"]').first();
      const manageButton = page.locator('button:has-text("Manage Team")').first();
    ...
    authenticatedTest("displays team details and members", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator('button:has-text("Members"), a:has-text("Members")')).toBeVisible();
      await expect(page.locator('button:has-text("Event Types"), a:has-text("Event Types")')).toBeVisible();
      await expect(page.locator('button:has-text("Settings"), a:has-text("Settings")')).toBeVisible();

      // Should show member list (including demo user as owner)
      await expect(page.locator('text="Demo", text="demo@workermill.com"')).toBeVisible();
    });

    authenticatedTest("can view team member list and roles", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");
      ...
      const memberElements = page.locator('[data-testid="team-member"], .member-card, div:has-text("@")');
      const memberCount = await memberElements.count();
      expect(memberCount).toBeGreaterThan(0);
    });

    authenticatedTest("can view team event types", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");

      // Click Event Types tab
      const eventTypesTab = page.locator('button:has-text("Event Types"), a:has-text("Event Types")');
      await eventTypesTab.click();

      // Should show team event types or empty state
      const eventTypesList = page.locator('[data-testid="event-type-card"], .event-type-card');
      const emptyState = page.locator('text="No team event types"');

      await expect(eventTypesList.first().or(emptyState)).toBeVisible();
    });

    authenticatedTest("can access team settings", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");
      ...
      await expect(page.locator('input[value="CalMill Demo Team"], input:has([value*="CalMill Demo Team"])').first()).toBeVisible();
      await expect(page.locator('input[value="calmill-demo-team"], input:has([value*="calmill-demo-team"])').first()).toBeVisible();
    });

    authenticatedTest("can edit team details", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator(`text="Updated Team ${timestamp}"`)).toBeVisible();
    });
  });

  test.describe("Team Public Page", () => {
    authenticatedTest("can view team public page", async ({ authenticatedPage: page }) => {
      ...
      const eventTypes = page.locator('[data-testid="event-type"], .event-type');
    ...
    test("public team page loads for unauthenticated users", async ({ page }) => {
  ...
  test.describe("Team Invitations", () => {
    authenticatedTest("can view team invitations", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");
      ...
      try {
        ...
        await expect(page.locator('text="OWNER", text="MEMBER", text="ADMIN"')).toBeVisible();
      }
    });

    authenticatedTest("can manage team member permissions", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");
      ...
      const roleManagement = page.locator('button:has-text("Change Role"), select, [data-testid="role-selector"]');
      const memberOptions = page.locator('[data-testid="member-options"], button:has([data-testid="menu-trigger"])');

      // Either should have role management or member options
      const hasManagement = await roleManagement.first().isVisible().catch(() => false);
      const hasOptions = await memberOptions.first().isVisible().catch(() => false);
  ...
  test.describe("Team Navigation", () => {
    authenticatedTest("can navigate between team sections", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");
    ...
    authenticatedTest("can navigate back to teams list", async ({ authenticatedPage: page }) => {
      await page.goto("/teams/calmill-demo-team");

      // Look for back/breadcrumb navigation
      const backLink = page.locator('a[href="/teams"]:has-text("Teams"), button:has-text("Back")');
  ...
  test.describe("Team Deletion", () => {
    authenticatedTest("can delete a team as owner", async ({ authenticatedPage: page }) => {
      ...
      await page.fill('input[id="team-name"]', teamName);
      await page.fill('input[id="team-slug"]', `delete-test-team-${timestamp}`);
      await page.click('button[type="submit"]:has-text("Create Team")');
      ...
      const deleteButton = page.locator('button:has-text("Delete Team"), button:has-text("Delete")');
      ...
      try {
        ...
        await expect(page.locator('input[value*="Delete Test Team"]')).toBeVisible();
      }
    });
  });
});