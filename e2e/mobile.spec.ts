...
test.describe('Mobile Responsive Layout', () => {
  ...
  test.describe('Public profile page on mobile', () => {
    ...
    test('event type cards are vertically stacked on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await goToProfilePage(page);
  ...
  test.describe('Booking page calendar on mobile', () => {
    ...
    test('calendar month navigation buttons are visible on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Dashboard sidebar on mobile', () => {
    ...
    test.fixme('dashboard home page loads on mobile viewport', async ({ page }) => {
      await setMobileViewport(page);
      await page.goto('/');
    ...
    test('dashboard sidebar is not blocking content on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await page.goto('/event-types');

      // Main content should be accessible on mobile
      await expect(page.locator('h1', { hasText: 'Event Types' })).toBeVisible({ timeout: 10_000 });
    });
    ...
    test('bookings page content is accessible on mobile', async ({ page }) => {
      ...
      await expect(page.locator('h1', { hasText: 'Bookings' })).toBeVisible({ timeout: 10_000 });
    });
  });
  ...
  test.describe('Booking form on mobile', () => {
    test.fixme('booking form inputs are accessible on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Dashboard home on mobile — no overflow', () => {
    ...
    test('dashboard home has no horizontal scroll on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await page.goto('/');
    ...
    test.fixme('stat cards are visible on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await page.goto('/');
  ...
  test.describe('Calendar month navigation on mobile', () => {
    test('calendar next month button is tappable on mobile', async ({ page }) => {
      await setMobileViewport(page);
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('time slot list is scrollable on mobile when many slots exist', async ({ page }) => {
      await setMobileViewport(page);
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
