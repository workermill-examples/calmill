...
test.describe('Public Booking Flow', () => {
  ...
  test.describe('Public profile page', () => {
    ...
    test('clicking an event type card navigates to booking page', async ({ page }) => {
      ...
      await expect(page).toHaveURL(new RegExp(`/${DEMO_USER.username}/${EVENT_30MIN.slug}`));
    });
  ...
  test.describe('Booking page — calendar', () => {
    ...
    test.fixme('shows event type title and duration on booking page', async ({ page }) => {
      await goToBookingPage(page);
    ...
    test('calendar shows month navigation buttons', async ({ page }) => {
      ...
      await expect(page.locator('button[aria-label="Go to previous month"]')).toBeVisible();
      await expect(page.locator('button[aria-label="Go to next month"]')).toBeVisible();
    });
    ...
    test('prompts to select a date before showing slots', async ({ page }) => {
      ...
      await expect(page.locator('text=Select a date to see available times')).toBeVisible();
    });
  });
  ...
  test.describe('Date selection and slot display', () => {
    test.fixme('selecting a date shows the slots panel', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('shows time slots after selecting an available date', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('clicking a slot highlights it and shows Confirm button', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Timezone switching', () => {
    ...
    test('timezone dropdown opens when clicked', async ({ page }) => {
      ...
      await expect(page.locator('input[aria-label="Search timezones"]')).toBeVisible({
        timeout: 3_000,
      });
    });

    test.fixme('can search for and select a different timezone', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Month navigation', () => {
    ...
    test('previous month navigation is available after going forward', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Slot selection and booking form', () => {
    test.fixme('confirming a slot shows the booking form', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('booking form shows the selected event title', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('booking form has a Go back button to return to calendar', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('going back from form returns to calendar view', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('shows validation errors when submitting empty form', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('successfully submits booking and redirects to confirmation', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Booking confirmation page', () => {
    ...
    test.fixme('shows success message on confirmation page', async ({ page }) => {
      await assertBookingConfirmed(page);
    });

    test.fixme('shows event type title on confirmation page', async ({ page }) => {
      await expect(page.locator('h2', { hasText: EVENT_30MIN.title })).toBeVisible();
    });

    test.fixme('shows attendee email on confirmation page', async ({ page }) => {
      await expect(page.locator(`text=${ATTENDEE.email}`)).toBeVisible();
    });

    test.fixme('shows attendee name on confirmation page', async ({ page }) => {
      await expect(page.locator(`text=${ATTENDEE.name}`)).toBeVisible();
    });

    test.fixme('shows booking reference UID on confirmation page', async ({ page }) => {
      await expect(page.locator(`text=${bookingUid}`)).toBeVisible();
    });

    test.fixme('shows Reschedule and Cancel links', async ({ page }) => {
      const rescheduleLink = page.locator(`a[href="/booking/${bookingUid}/reschedule"]`);
    ...
    test.fixme('shows Add to Calendar section', async ({ page }) => {
      await expect(page.locator('h3', { hasText: 'Add to Calendar' })).toBeVisible();
    });
  });
  ...
  test.describe('Add to Calendar links', () => {
    ...
    test.fixme('Google Calendar button generates a valid Google Calendar link', async ({
      page,
    }) => {
    ...
    test.fixme('Outlook calendar download link is visible', async ({ page }) => {
    ...
    test.fixme('Apple Calendar download link is visible', async ({ page }) => {
  ...
  test.describe('Cancel flow', () => {
    ...
    test.fixme('cancel page loads with booking details', async ({ page }) => {
      await goToCancelPage(page, bookingUid);

      await expect(page.locator('h1', { hasText: 'Cancel Meeting' })).toBeVisible();
      await expect(page.locator(`text=${EVENT_30MIN.title}`)).toBeVisible();
    });

    test.fixme('cancel page shows warning message', async ({ page }) => {
      await goToCancelPage(page, bookingUid);
    ...
    test.fixme('cancel page has Go Back link to booking details', async ({ page }) => {
      await goToCancelPage(page, bookingUid);
    ...
    test.fixme('submitting cancellation shows success state', async ({ page }) => {
      await goToCancelPage(page, bookingUid);
    ...
    test.fixme('successful cancellation shows rebook link', async ({ page }) => {
      await goToCancelPage(page, bookingUid);
    ...
    test.fixme('cancelled booking shows cancelled status on confirmation page', async ({
      page,
    }) => {
      await goToCancelPage(page, bookingUid);
      await submitCancellation(page);
  ...
  test.describe('Reschedule flow', () => {
    ...
    test.fixme('reschedule page loads with heading', async ({ page }) => {
      await goToReschedulePage(page, bookingUid);

      await expect(page.locator('h1', { hasText: 'Reschedule your meeting' })).toBeVisible();
    });

    test.fixme('reschedule page shows calendar for new time selection', async ({ page }) => {
      await goToReschedulePage(page, bookingUid);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('reschedule page shows original booking time crossed out', async ({ page }) => {
      await goToReschedulePage(page, bookingUid);
  ...
  test.describe('404 handling', () => {
    ...
    test('invalid event type slug for existing user returns 404', async ({ page }) => {
      const response = await page.goto(`/${DEMO_USER.username}/this-event-does-not-exist`);
      expect(response?.status()).toBe(404);
    });
    ...
    test('invalid UID for cancel page returns 404', async ({ page }) => {
      const response = await page.goto('/booking/invalid-uid-that-does-not-exist/cancel');
      expect(response?.status()).toBe(404);
    });

    test('invalid UID for reschedule page returns 404', async ({ page }) => {
      const response = await page.goto('/booking/invalid-uid-that-does-not-exist/reschedule');
      expect(response?.status()).toBe(404);
    });
  });
  ...
  test.describe('Empty state handling', () => {
    test.fixme("shows 'no available times' message when date has no slots", async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
    ...
    test('profile page does not show empty state for demo user', async ({ page }) => {
      await goToProfilePage(page);
  ...
  test.describe('Mobile responsive layout', () => {
    ...
    test.fixme('booking form is usable on mobile', async ({ page }) => {
      await goToBookingPage(page);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Loading states', () => {
    ...
    test.fixme('Schedule Meeting button shows loading state during submission', async ({
      page,
    }) => {
      ...
      await expect(page.locator('button[type="submit"]', { hasText: /Scheduling/ })).toBeVisible({
        timeout: 3_000,
      });
    });
  });
  ...
  test.describe('URL query param pre-selection', () => {
    test('booking page pre-selects date from URL query param', async ({ page }) => {
      ...
      await page.goto(`/${DEMO_USER.username}/${EVENT_30MIN.slug}?date=${dateStr}`);
      await waitForCalendarToLoad(page);
    ...
    test.fixme('can book a 60 minute event type', async ({ page }) => {
      await goToBookingPage(page, DEMO_USER.username, EVENT_60MIN.slug);
      await waitForCalendarToLoad(page);

      await expect(page.locator('text=60 min')).toBeVisible();
      await expect(page.locator('h1', { hasText: EVENT_60MIN.title })).toBeVisible();
    });
  });
});
