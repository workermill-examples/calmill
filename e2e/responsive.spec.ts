...
test.describe("Responsive Design", () => {
  ...
  test.describe("320px Mobile Viewport", () => {
    ...
    test("navigation is responsive at 320px", async ({ page }) => {
      ...
      const mobileNav = page.locator(
        '[data-testid="mobile-nav"], .mobile-nav, button[aria-label="Toggle menu"]',
      );
    ...
    test("event types list is responsive at 320px", async ({ page }) => {
      ...
      const eventTypeCards = page.locator(
    ...
    test("bookings page is responsive at 320px", async ({ page }) => {
      ...
      const bookingsTable = page.locator(
        '[data-testid="bookings-table"], table, .bookings-list',
      );
      ...
      if ((await bookingsTable.count()) > 0) {
        ...
        if (tableBox) {
          ...
          const container = page
            .locator(".table-container, .overflow-x-auto")
            .first();
      ...
      const filters = page.locator(
        '[data-testid="booking-filters"], .filter-controls',
      );
      ...
      if ((await buttons.count()) > 0) {
        for (let i = 0; i < Math.min(5, await buttons.count()); i++) {
          ...
          if (buttonBox) {
            ...
            ).toBeGreaterThanOrEqual(40); // 40px minimum for better UX
          }
        }
      }
    });
  ...
  test.describe("768px Tablet Viewport", () => {
    ...
    test("event type editor is responsive on tablet", async ({ page }) => {
      ...
      const firstEventType = page
        .locator('[data-testid="event-type-card"], .event-type-card')
        .first();
      ...
      if ((await firstEventType.count()) > 0) {
        ...
        const editor = page.locator(
          '[data-testid="event-type-editor"], .event-type-editor, form',
        );
    ...
    test("booking interface is tablet-friendly", async ({ page }) => {
      ...
      const bookingInterface = page
        .locator('[data-testid="booking-interface"], .booking-container')
        .first();
      ...
      const calendar = page.locator(
        '[data-testid="calendar-picker"], .calendar',
      );
  ...
  test.describe("1024px Desktop Viewport", () => {
    ...
    test("event types grid layout on desktop", async ({ page }) => {
      ...
      const eventTypeGrid = page.locator(
        '[data-testid="event-types-grid"], .grid',
      );
    ...
    test("bookings table is fully visible on desktop", async ({ page }) => {
      ...
      const bookingsTable = page.locator(
        '[data-testid="bookings-table"], table',
      );
      ...
      if ((await bookingsTable.count()) > 0) {
        ...
        const tableHeaders = page.locator(
          'th:visible, [data-testid="table-header"]:visible',
        );
    ...
    test("multi-column layout works in settings", async ({ page }) => {
      ...
      const settingsContainer = page
        .locator(".settings-container, .grid, main")
        .first();
  ...
  test.describe("Content reflow", () => {
    test("dashboard content reflows properly when resizing", async ({
      page,
    }) => {
      // Start with desktop
      await page.setViewportSize({ width: 1024, height: 768 });
