...
test.describe("Public Booking Flow", () => {
  ...
  test("complete public booking flow: visit /demo/quick-chat, select date/time, submit form, see confirmation, download .ics", async ({
    ...
    await expect(page).toHaveTitle(/Book.*Quick Chat.*with.*Alex Demo.*CalMill/);
    await expect(page.locator("h1, h2")).toContainText("Quick Chat");
    await expect(page.locator("[data-testid]").or(page.locator("text=15"))).toBeVisible();
    ...
    const calendar = page.locator("[data-testid='calendar-picker']").or(
      page.locator(".calendar").or(
        page.locator("button").filter({ hasText: /^\d{1,2}$/ })
      )
    );
    ...
    for (let i = 0; i < Math.min(buttonCount, 10); i++) {
      ...
      const hasDisabledClass = await button.getAttribute("class").then(
        (className) => className?.includes("disabled") || className?.includes("opacity-50")
      ).catch(() => false);
    ...
    await page.fill("input[name='name'], input[name='attendeeName']", "John Doe");
    await page.fill(
  ...
  test("public booking flow handles missing required fields", async ({
    ...
    const nameField = page.locator("input[name='name'], input[name='attendeeName']");
  ...
  test("public booking flow handles timezone selection", async ({ page }) => {
    ...
    const timezoneSelector = page.locator(
      "select[name='timezone'], [data-testid='timezone-select']"
    ).or(page.locator("button").filter({ hasText: /America|UTC|GMT|timezone/i }));

    if (await timezoneSelector.count() > 0) {
      await timezoneSelector.click();

      // Select a different timezone if dropdown opens
      const timezoneOptions = page.locator("option").or(
        page.locator("[role='option']")
      );

      if (await timezoneOptions.count() > 0) {
        await timezoneOptions.filter({ hasText: /Pacific|Los_Angeles/ }).first().click();
      }
    }
  ...
  test("public booking form renders custom questions correctly", async ({
    ...
    const topicField = page.locator("input[name='topic'], textarea[name='topic']");
