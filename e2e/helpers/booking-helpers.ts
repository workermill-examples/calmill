 ...
 */
export async function clickEventTypeCard(
  page: Page,
  title: string
): Promise<void> {
...
export function getAvailableDayButtons(page: Page): Locator {
  return page
    .locator(
      '[aria-label^="Calendar"] [aria-label][role="gridcell"] button:not([disabled])'
    )
...
export async function submitBookingForm(page: Page): Promise<void> {
  await page
    .locator('button[type="submit"]', { hasText: "Schedule Meeting" })
    .click();
}
 ...
 */
export async function goToConfirmationPage(
  page: Page,
  uid: string
): Promise<void> {
 ...
 */
export async function goToReschedulePage(
  page: Page,
  uid: string
): Promise<void> {
