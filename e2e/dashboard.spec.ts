...
test.describe("Dashboard Flows", () => {
  ...
  test.describe("Dashboard home", () => {
    ...
    test("dashboard home shows Upcoming Bookings section", async ({ page }) => {
      ...
      await expect(
        page.locator("h2", { hasText: "Upcoming Bookings" })
      ).toBeVisible();
    });
  ...
  test.describe("Event types list", () => {
    ...
    test("event types list shows existing event types from seed", async ({
      page,
    }) => {
      await page.goto("/event-types");
  ...
  test.describe("Event type creation", () => {
    ...
    test("clicking New Event Type opens the create dialog", async ({
      page,
    }) => {
  ...
  test.describe("Event type editor", () => {
    ...
    test("event type editor shows General tab by default", async ({ page }) => {
      await expect(
        page.locator("button", { hasText: "General" })
      ).toBeVisible();
    });
    ...
    test("event type editor has all five tabs", async ({ page }) => {
      ...
      await expect(
        page.locator("button", { hasText: "Availability" })
      ).toBeVisible();
      await expect(
        page
          .locator("button", { hasText: "Limits" })
          .or(
            page.locator("button", { hasText: "Limits & Buffers" })
          )
      ).toBeVisible();
      await expect(
        page.locator("button", { hasText: "Booking Form" })
      ).toBeVisible();
      await expect(
    ...
    test("clicking Availability tab shows availability content", async ({
      page,
    }) => {
      ...
      await expect(
        page
          .locator("text=Schedule")
          .or(page.locator("text=Availability"))
          .first()
    ...
    test("clicking Limits tab shows limits content", async ({ page }) => {
      const limitsTabName =
        (await page
          .locator("button", { hasText: "Limits & Buffers" })
          .count()) > 0
          ? "Limits & Buffers"
          : "Limits";
  ...
  test.describe("Event type active toggle", () => {
    ...
    test("event type card has an active/inactive toggle switch", async ({
      page,
    }) => {
  ...
  test.describe("Event type delete flow", () => {
    ...
    test("delete button opens a confirmation dialog", async ({ page }) => {
      const deleteButton = page.locator(
        `button[aria-label="Delete ${throwawayTitle}"]`
      );
    ...
    test("confirming delete removes the event type from the list", async ({
      page,
    }) => {
      const deleteButton = page.locator(
        `button[aria-label="Delete ${throwawayTitle}"]`
      ...
      if ((await deleteButton.count()) > 0) {
        ...
        await expect(
          page.locator(`text="${throwawayTitle}"`)
    ...
    test("cancelling delete dialog keeps the event type", async ({ page }) => {
      ...
      if ((await deleteButton.count()) > 0) {
        ...
        await page
          .locator('[role="dialog"] button', { hasText: /^Cancel$/ })
          .click();
  ...
  test.describe("Bookings list tabs", () => {
    ...
    test("bookings page has Past and Cancelled tabs", async ({ page }) => {
      await expect(
        page.locator("button", { hasText: "Past" })
      ).toBeVisible();
      await expect(
        page.locator("button", { hasText: "Cancelled" })
      ).toBeVisible();
    });
    ...
    test("clicking Cancelled tab switches to cancelled bookings", async ({
      page,
    }) => {
    ...
    test("bookings list shows empty state or booking cards", async ({
      page,
    }) => {
  ...
  test.describe("Booking status actions", () => {
    ...
    test("Accept button is visible on pending bookings if any exist", async ({
      page,
    }) => {
    ...
    test("clicking Accept on a pending booking updates its status", async ({
      page,
    }) => {
      const acceptButton = page
        .locator("button", { hasText: "Accept" })
    ...
    test("Cancel button is visible on accepted bookings if any exist", async ({
      page,
    }) => {
  ...
  test.describe("Availability schedule editor", () => {
    ...
    test("availability page shows schedule name or heading", async ({
      page,
    }) => {
    ...
    test("availability page shows weekly grid with day names", async ({
      page,
    }) => {
      await expect(
        page
    ...
    test("availability page shows toggle switches for each day", async ({
      page,
    }) => {
      const toggles = page.locator('button[role="switch"]');
