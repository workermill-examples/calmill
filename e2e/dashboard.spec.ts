...
test.describe("Dashboard", () => {
  ...
  test.describe("Authenticated Dashboard", () => {
    authTest("loads dashboard with correct content", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator("text=Overview of your scheduling activity")).toBeVisible();
    });

    authTest("displays stat cards with correct titles", async ({ authenticatedPage: page }) => {
    ...
    authTest("stat cards show numeric values", async ({ authenticatedPage: page }) => {
      ...
      const statCards = page.locator('[role="group"]:has(text="Upcoming"), [role="group"]:has(text="Pending"), [role="group"]:has(text="This Month"), [role="group"]:has(text="Popular")');

      // Verify at least one stat card shows a non-zero number
      const upcomingCard = page.locator('text="Upcoming"').locator('../..');
      const upcomingValue = await upcomingCard.locator('.text-2xl').textContent();
      ...
      const pendingValue = await pendingCard.locator('.text-2xl').textContent();
      expect(pendingValue).toMatch(/^\d+$/);

      const thisMonthCard = page.locator('text="This Month"').locator('../..');
      const thisMonthValue = await thisMonthCard.locator('.text-2xl').textContent();
    ...
    authTest("charts render without errors", async ({ authenticatedPage: page }) => {
      // Wait for dashboard to load
      await page.waitForSelector('text="Dashboard"');
      await page.waitForTimeout(3000); // Allow time for charts to render

      // Check that chart containers exist
      const lineChart = page.locator('text="Bookings Over Time"').locator('../..');
    ...
    authTest("displays upcoming bookings section", async ({ authenticatedPage: page }) => {
      // Wait for dashboard to load
      await page.waitForSelector('text="Dashboard"');
    ...
    authTest("upcoming bookings list is populated", async ({ authenticatedPage: page }) => {
      // Wait for dashboard to load
      await page.waitForSelector('text="Dashboard"');
      await page.waitForTimeout(2000);

      const upcomingSection = page.locator('text="Upcoming Bookings"').locator('../..');

      // Should either show bookings or "No upcoming bookings" message
      const hasBookings = await page.locator('[data-testid="upcoming-booking-item"]').count() > 0;
      const hasNoBookingsMessage = await page.locator('text="No upcoming bookings"').isVisible();
      ...
      if (hasBookings) {
        const firstBooking = page.locator('[data-testid="upcoming-booking-item"]').first();

        // Should have attendee name
        await expect(firstBooking.locator('.text-sm.font-medium')).toBeVisible();

        // Should have date and time
        await expect(firstBooking.locator('.text-sm.font-medium', { hasText: /\w{3} \d{1,2}/ })).toBeVisible();
        await expect(firstBooking.locator('.text-xs.text-muted-foreground', { hasText: /\d{1,2}:\d{2}/ })).toBeVisible();
      }
    });

    authTest("navigates to bookings page from 'View All' link", async ({ authenticatedPage: page }) => {
      // Wait for dashboard to load
      await page.waitForSelector('text="Dashboard"');
    ...
    authTest("handles loading states properly", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator('[data-testid="dashboard-skeleton"]')).not.toBeVisible({ timeout: 10000 });
    });

    authTest("displays trend information in stat cards", async ({ authenticatedPage: page }) => {
      // Wait for dashboard to load
      await page.waitForSelector('text="Dashboard"');
      await page.waitForTimeout(2000);
  ...
  test.describe("Responsive Design", () => {
    authTest("dashboard is responsive on mobile", async ({ authenticatedPage: page }) => {
      // Set mobile viewport
    ...
    authTest("dashboard is responsive on tablet", async ({ authenticatedPage: page }) => {
      // Set tablet viewport
  ...
  test.describe("Accessibility", () => {
    authTest("dashboard has proper heading structure", async ({ authenticatedPage: page }) => {
      await page.waitForSelector('text="Dashboard"');
      ...
      await expect(page.locator('h3, [role="heading"]:has-text("Bookings Over Time")')).toBeVisible();
      await expect(page.locator('h3, [role="heading"]:has-text("By Event Type")')).toBeVisible();
      await expect(page.locator('h3, [role="heading"]:has-text("By Status")')).toBeVisible();
      await expect(page.locator('h3, [role="heading"]:has-text("Upcoming Bookings")')).toBeVisible();
    });
