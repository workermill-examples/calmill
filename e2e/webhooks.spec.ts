...
test.describe("Webhook Management", () => {
  ...
  test.describe("Webhooks settings page", () => {
    ...
    test("webhooks settings page loads at /settings/webhooks", async ({
      page,
    }) => {
    ...
    test("webhooks settings page shows a heading or webhook-related content", async ({
      page,
    }) => {
      await page.goto("/settings/webhooks");
      ...
      const webhookHeading = page
        .locator("h1, h2", { hasText: /Webhooks?/i })
        .first();
      const settingsHeading = page
        .locator("h1, h2", { hasText: /Settings?/i })
        .first();
    ...
    test("webhooks settings page shows button to add a new webhook", async ({
      page,
    }) => {
      await page.goto("/settings/webhooks");
      await page.waitForTimeout(2_000);
  ...
  test.describe("Create webhook via API", () => {
    test("POST /api/webhooks creates a webhook with valid data", async ({
      page,
    }) => {
  ...
  test.describe("Webhook detail and update via API", () => {
    ...
    test("GET /api/webhooks/[id] returns webhook detail with deliveries", async ({
      page,
    }) => {
      if (!webhookId) {
    ...
    test("PUT /api/webhooks/[id] can toggle active state", async ({ page }) => {
      ...
      // Re-activate
      const reactivateResponse = await page.request.put(
        `/api/webhooks/${webhookId}`,
        {
          data: { active: true },
        }
  ...
  test.describe("Webhook test delivery", () => {
    ...
    test("POST /api/webhooks/[id]/test sends a test payload", async ({
      page,
    }) => {
      if (!webhookId) {
  ...
  test.describe("Webhooks settings UI", () => {
    ...
    test("settings page navigation shows webhooks link or section", async ({
      page,
    }) => {
      ...
      if ((await webhooksLink.count()) > 0) {
        ...
        await expect(
          page.locator("h1, h2", { hasText: /Settings?/i }).first()
        ).toBeVisible({ timeout: 10_000 });
      }
    });
  });
