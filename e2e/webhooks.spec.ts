...
test.describe("Webhooks Management", () => {
  test.describe("Webhooks Settings Page", () => {
    authenticatedTest("loads webhooks page and shows existing webhooks", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator("text=Receive real-time notifications when booking events occur")).toBeVisible();
      ...
      const webhookCards = page.locator('[data-testid="webhook-card"], .border.border-border.rounded-lg');
    ...
    authenticatedTest("can create a new webhook", async ({ authenticatedPage: page }) => {
      ...
      const activeToggle = page.locator('input[type="checkbox"], button[role="switch"]').filter({ hasText: /active/i }).or(
        page.locator('[data-testid="active-toggle"]')
      );
      ...
      await expect(page.locator('text="Create New Webhook"')).not.toBeVisible();
    ...
    authenticatedTest("validates webhook creation form", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      await expect(page.locator('text="Please select at least one event"')).toBeVisible();
    ...
    authenticatedTest("shows all available webhook events", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      await expect(page.locator('text="When a new booking is made"')).toBeVisible();
      await expect(page.locator('text="When you accept a pending booking"')).toBeVisible();
    });
  });

  test.describe("Webhook Management", () => {
    authenticatedTest("can edit an existing webhook", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      // Create a webhook first if none exist
      const editButton = page.locator('button:has-text("Edit")');
      const isEditVisible = await editButton.first().isVisible().catch(() => false);

      if (!isEditVisible) {
        // Create a webhook first
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://edit-test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
    ...
    authenticatedTest("can test webhook delivery", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      // Look for existing webhook or create one
      const testButton = page.locator('button:has-text("Test")');
      const isTestVisible = await testButton.first().isVisible().catch(() => false);

      if (!isTestVisible) {
        // Create a webhook first
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
      ...
      await page.waitForSelector('text="Testing..."', { state: 'detached', timeout: 15000 });
    ...
    authenticatedTest("can delete a webhook", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
    ...
    authenticatedTest("can toggle webhook active state", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      // Create a webhook first if none exist
      const existingWebhook = page.locator('.border.border-border.rounded-lg').first();
      const hasWebhook = await existingWebhook.isVisible().catch(() => false);

      if (!hasWebhook) {
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://toggle-test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
      ...
      const activeToggle = page.locator('[data-testid="active-toggle"], input[type="checkbox"]').last();
  ...
  test.describe("Webhook Secrets and Security", () => {
    authenticatedTest("displays webhook signing secret", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      // Create webhook if none exist
      const hasWebhook = await page.locator('.border.border-border.rounded-lg').first().isVisible().catch(() => false);

      if (!hasWebhook) {
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://secret-test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
      ...
      await expect(page.locator('code').filter({ hasText: /^[a-f0-9]{64}$/ })).toBeVisible();
    ...
    authenticatedTest("can copy webhook secret to clipboard", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      if (!hasWebhook) {
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://copy-test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
      ...
      await expect(page.locator('text="Copied", text="copied to clipboard"')).toBeVisible();
    });

    authenticatedTest("shows event trigger badges", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      // Create webhook with multiple events
      await page.click('button:has-text("Add Webhook")');
      await page.fill('input[id="webhook-url"]', `https://events-test.example.com/${Date.now()}`);
  ...
  test.describe("Webhook Delivery History", () => {
    authenticatedTest("shows delivery count for webhooks", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      if (!hasWebhook) {
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://delivery-test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
    ...
    authenticatedTest("shows webhook creation date", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      if (!hasWebhook) {
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://date-test.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
  ...
  test.describe("Webhook Form Validation", () => {
    authenticatedTest("validates URL format in create form", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      for (const invalidUrl of invalidUrls) {
        ...
        const hasValidationError = await page.locator('text="Please enter a valid URL", text="URL is required"').isVisible();
    ...
    authenticatedTest("requires at least one event selection", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      await page.click('button:has-text("Add Webhook")');

      // Fill valid URL but no events
      await page.fill('input[id="webhook-url"]', "https://valid-url.example.com");
    ...
    authenticatedTest("accepts valid webhook configurations", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
  ...
  test.describe("Webhook Loading States", () => {
    authenticatedTest("shows loading state when creating webhook", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      await page.click('button:has-text("Add Webhook")');

      await page.fill('input[id="webhook-url"]', `https://loading-test.example.com/${Date.now()}`);
      await page.check('input[id="create-event-BOOKING_CREATED"]');
      ...
      const loadingText = await page.locator('text="Creating..."').isVisible().catch(() => false);

      // Either we caught the loading state, or it completed very quickly
      expect(typeof loadingText).toBe('boolean');
    });

    authenticatedTest("shows loading state when testing webhook", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");

      // Ensure we have a webhook to test
      const hasWebhook = await page.locator('button:has-text("Test")').first().isVisible().catch(() => false);

      if (!hasWebhook) {
        await page.click('button:has-text("Add Webhook")');
        await page.fill('input[id="webhook-url"]', `https://test-loading.example.com/${Date.now()}`);
        await page.check('input[id="create-event-BOOKING_CREATED"]');
        await page.click('button:has-text("Create Webhook")');
      }
    ...
    authenticatedTest("handles webhook loading errors gracefully", async ({ authenticatedPage: page }) => {
      await page.goto("/settings/webhooks");
      ...
      const errorState = await page.locator('text="Failed to load webhooks"').isVisible().catch(() => false);
      const loadingState = await page.locator('text="Loading"').isVisible().catch(() => false);
      const contentState = await page.locator('button:has-text("Add Webhook")').isVisible().catch(() => false);
