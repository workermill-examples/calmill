...
test.describe("Embed Functionality", () => {
  ...
  test("embed code generator page loads", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Embed Code Generator", exact: true }),
    ).toBeVisible();

    // Check description is present
    await expect(
      page.locator("text=Generate embeddable booking widgets for your website"),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: "Generated Code", exact: true }),
    ).toBeVisible();
  ...
  test("event type selection works", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="event-type-selector"]', {
      timeout: 10000,
    });

    // Should have at least one event type option
    const eventTypeSelector = page.locator(
  ...
  test("embed mode toggle works", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="embed-mode-selector"]', {
      timeout: 10000,
    });
  ...
  test("live preview iframe loads", async ({ page }) => {
    ...
    await page.waitForSelector('iframe[title="CalMill Embed Preview"]', {
      timeout: 10000,
    });
  ...
  test("open full page button works", async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Open Full Page")', {
      timeout: 10000,
    });

    const openFullPageButton = page.getByRole("button", {
      name: "Open Full Page",
      exact: true,
    });
  ...
  test("integration instructions are displayed", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Use inline embeds for a seamless experience"),
    ).toBeVisible();
    await expect(
      page.locator("text=Listen for the `calmillBooking` event"),
    ).toBeVisible();
  });
  ...
  test("popup mode preview works", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Click the button to test the popup experience"),
    ).toBeVisible();
  ...
  test("embed code contains proper CORS headers reference", async ({
    page,
  }) => {
    await page.goto("/embed");
    ...
    expect(codeText).toContain("data-calmill-embed") ||
      expect(codeText).toContain("data-calmill-popup");
  });
