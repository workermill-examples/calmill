...
test.describe('Embed Widget', () => {
  ...
  test.describe('Inline embed page rendering', () => {
    test('embed booking page loads at /embed/[username]/[slug]', async ({ page }) => {
    ...
    test('embed booking page shows calendar for selecting dates', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}`);
      await waitForCalendarToLoad(page);
  ...
  test.describe('Embed theme query param', () => {
    test('embed page accepts ?theme=dark query param', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}?theme=dark`);

      // Should load without error
      await expect(page.locator('h1').first()).toBeVisible({ timeout: 15_000 });
    });

    test('embed page with dark theme sets data-calmill-theme attribute', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}?theme=dark`);
    ...
    test('embed page defaults to light theme without theme param', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}`);
  ...
  test.describe('Embed script', () => {
    test('embed script is accessible at /embed/calmill-embed.js', async ({ page }) => {
  ...
  test.describe('postMessage resize event', () => {
    test('embed page sends calmill:resize postMessage on load', async ({ page }) => {
  ...
  test.describe('Booking via embed page', () => {
    test.fixme('can complete a full booking flow on the embed page', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}`);
      await waitForCalendarToLoad(page);
    ...
    test('embed page with ?hideEventDetails=true still shows calendar', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}?hideEventDetails=true`);
      await waitForCalendarToLoad(page);
    ...
    test('embed page accepts ?timezone query param', async ({ page }) => {
      await page.goto(`/embed/${DEMO_USER.username}/${EVENT_30MIN.slug}?timezone=Europe/London`);
