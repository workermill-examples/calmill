...
test.describe("Settings Management", () => {
  ...
  test.describe("Profile Settings", () => {
    authTest(
      "loads settings page with correct content",
  ...
  test.describe("Preferences Settings", () => {
    ...
    authTest(
      "can detect timezone automatically",
      async ({ authenticatedPage: page }) => {
        await page.goto("/settings/preferences");
        ...
        if (await detectButton.isVisible()) {
          ...
          await expect(page.locator('text="Timezone Updated"')).toBeVisible({
            timeout: 3000,
          });
        }
      },
  ...
  test.describe("Password Settings", () => {
    authTest(
      "can navigate to password settings",
      async ({ authenticatedPage: page }) => {
        await page.goto("/settings");
    ...
    authTest(
      ...
      async ({ authenticatedPage: page }) => {
        ...
        await expect(
          page.locator('text="At least one uppercase letter"'),
        ).toBeVisible();
        await expect(
          page.locator('text="At least one lowercase letter"'),
        ).toBeVisible();
      },
    );
    ...
    authTest(
      ...
      async ({ authenticatedPage: page }) => {
        ...
        await expect(
          page.locator('[data-testid="requirement-length"].text-red-500'),
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="requirement-uppercase"].text-red-500'),
        ).toBeVisible();
        ...
        await expect(
          page.locator('[data-testid="requirement-length"].text-green-500'),
        ).toBeVisible();
        await expect(
          page.locator('[data-testid="requirement-uppercase"].text-green-500'),
        ).toBeVisible();
      },
    );
    ...
    authTest(
      ...
      async ({ authenticatedPage: page }) => {
        ...
        const toggleButton = page.locator(
          'button[aria-label="Show password"]:near(input[name="currentPassword"])',
        );
    ...
    authTest(
      "validates current password is required",
      async ({ authenticatedPage: page }) => {
        await page.goto("/settings/password");
        ...
        // Check for validation error
        await expect(
          page.locator('text="Current password is required"'),
        ).toBeVisible({ timeout: 3000 });
      },
    );
  ...
  test.describe("Settings Navigation", () => {
    authTest(
      "can navigate between settings sections",
      async ({ authenticatedPage: page }) => {
        await page.goto("/settings");

        // Check that all navigation links are present
        await expect(
          page.locator('a[href="/settings"]:has(text("Profile"))'),
        ).toBeVisible();
        await expect(
          page.locator(
            'a[href="/settings/preferences"]:has(text("Preferences"))',
          ),
        ).toBeVisible();
        await expect(
          page.locator('a[href="/settings/password"]:has(text("Password"))'),
        ).toBeVisible();
        await expect(
          page.locator('a[href="/settings/calendars"]:has(text("Calendars"))'),
        ).toBeVisible();
        await expect(
          page.locator('a[href="/settings/webhooks"]:has(text("Webhooks"))'),
        ).toBeVisible();
    ...
    authTest(
      "shows active navigation state",
      async ({ authenticatedPage: page }) => {
        await page.goto("/settings");

        // Check that Profile is active
        const profileLink = page.locator(
          'a[href="/settings"]:has(text("Profile"))',
        );
        ...
        await expect(preferencesLink).toHaveClass(
    ...
    authTest(
      "has consistent layout across settings pages",
      async ({ authenticatedPage: page }) => {
        const settingsPages = [
          "/settings",
  ...
  test.describe("Form State Management", () => {
    ...
    authTest("handles loading states", async ({ authenticatedPage: page }) => {
      ...
      await expect(
        page.locator('[data-testid="settings-skeleton"]'),
      ).toBeVisible();
