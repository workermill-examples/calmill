...
test.describe('Recurring Bookings', () => {
  ...
  test.describe('Recurring tab in event type editor', () => {
    ...
    test('clicking Recurring tab shows recurring enable toggle', async ({ page }) => {
    ...
    test('recurring tab shows disabled state message when toggle is off', async ({ page }) => {
      await page.locator('button', { hasText: 'Recurring' }).click();
      ...
      if (initialChecked === 'false') {
        await expect(page.locator('text=Recurring bookings are disabled')).toBeVisible({
          timeout: 5_000,
        });
      } else {
  ...
  test.describe('Enable recurring toggle', () => {
    ...
    test.fixme('enabling recurring shows frequency options', async ({ page }) => {
      const toggle = page.locator('button[role="switch"]').first();
      ...
      const frequencySection = page.locator('text=Frequency').or(weeklyOption).first();
  ...
  test.describe('Recurring frequency options', () => {
    ...
    test.fixme('Weekly frequency option is visible', async ({ page }) => {
      await expect(page.locator('button', { hasText: 'Weekly' })).toBeVisible({ timeout: 5_000 });
    });

    test.fixme('Biweekly frequency option is visible', async ({ page }) => {
      await expect(page.locator('button', { hasText: 'Biweekly' })).toBeVisible({ timeout: 5_000 });
    });

    test.fixme('Monthly frequency option is visible', async ({ page }) => {
      await expect(page.locator('button', { hasText: 'Monthly' })).toBeVisible({ timeout: 5_000 });
    });

    test.fixme('clicking Biweekly selects it as the active frequency', async ({ page }) => {
  ...
  test.describe('Maximum occurrences input', () => {
    ...
    test.fixme('max occurrences input is visible when recurring is enabled', async ({ page }) => {
      const occurrencesInput = page.locator('input[aria-label="Maximum occurrences"]');
      await expect(occurrencesInput).toBeVisible({ timeout: 5_000 });
    });

    test.fixme('max occurrences input accepts a number value', async ({ page }) => {
      const occurrencesInput = page.locator('input[aria-label="Maximum occurrences"]');
      await expect(occurrencesInput).toBeVisible({ timeout: 5_000 });
  ...
  test.describe('Recurring booking preview', () => {
    ...
    test.fixme('preview text shows current max occurrences', async ({ page }) => {
      const preview = page.locator('text=Attendees can book up to');
      await expect(preview).toBeVisible({ timeout: 5_000 });
    });

    test.fixme('preview text updates when max occurrences changes', async ({ page }) => {
      const occurrencesInput = page.locator('input[aria-label="Maximum occurrences"]');
      await expect(occurrencesInput).toBeVisible({ timeout: 5_000 });
  ...
  test.describe('Cancel a booking', () => {
    ...
    test.fixme('cancel page for a booking loads correctly', async ({ page }) => {
    ...
    test.fixme('can cancel a booking successfully', async ({ page }) => {
      await goToCancelPage(page, bookingUid);
