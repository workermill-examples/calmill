...
test.describe('Calendar Integration', () => {
  ...
  test('calendar settings page loads', async ({ page }) => {
    ...
    // Check description
    await expect(page.locator('text=Connect your calendar accounts to enable scheduling conflicts')).toBeVisible();

    // Check Google Calendar section
    await expect(page.getByRole('heading', { name: 'Google Calendar', exact: true })).toBeVisible();
    await expect(page.locator('text=Connect your Google Calendar to prevent double bookings')).toBeVisible();
  });

  test('google calendar not configured warning displays when applicable', async ({ page }) => {
    await page.goto('/settings/calendars');
    ...
    const warningElement = page.locator('text=Google Calendar Integration Not Configured');
    ...
    if (await warningElement.count() > 0) {
      ...
      await expect(page.locator('text=The server administrator needs to configure Google Calendar')).toBeVisible();

      // Connect button should be disabled
      const connectButton = page.getByRole('button', { name: 'Connect Google Calendar', exact: true });
  ...
  test('connection status badge displays correctly', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="connection-status"], .badge, text=Connected, text=Not Connected', { timeout: 10000 });
  ...
  test('google oauth connect flow initiates', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Connect Google Calendar")', { timeout: 10000 });
  ...
  test('connected state shows management options', async ({ page }) => {
    ...
    if (await connectedBadge.count() > 0) {
      ...
      await expect(page.getByRole('button', { name: 'Refresh Calendars', exact: true })).toBeVisible();
      await expect(page.getByRole('button', { name: 'Disconnect', exact: true })).toBeVisible();

      // Should show calendars section
      await expect(page.getByRole('heading', { name: 'Available Calendars', exact: true })).toBeVisible();
    }
  });
  ...
  test('refresh calendars functionality', async ({ page }) => {
    ...
    if (await refreshButton.count() > 0) {
      ...
      await page.waitForSelector('text=Refreshing...', { state: 'hidden', timeout: 10000 });
  ...
  test('disconnect flow works', async ({ page }) => {
    ...
    if (await disconnectButton.count() > 0) {
      ...
      await expect(page.locator('text=Disconnecting...')).toBeVisible({ timeout: 5000 });

      // Wait for disconnect to complete
      await page.waitForSelector('text=Disconnecting...', { state: 'hidden', timeout: 10000 });
  ...
  test('calendar list displays when connected', async ({ page }) => {
    ...
    if (await connectedBadge.count() > 0) {
      ...
      const calendarList = page.locator('[data-testid="calendar-list"], .calendar-item');
      ...
      if (hasCalendars) {
        ...
        await expect(page.locator('.w-4.h-4.rounded, [data-testid="calendar-color"]').first()).toBeVisible();
      }
  ...
  test('calendar badges display correctly', async ({ page }) => {
    ...
    if (await connectedBadge.count() > 0) {
      ...
      const accessRoleBadge = page.locator('[data-testid="access-role-badge"], .badge:has-text("owner"), .badge:has-text("reader"), .badge:has-text("writer")').first();

      // If calendars exist, should have access role badges
      const calendarItems = page.locator('.calendar-item, [data-testid="calendar-item"]');
  ...
  test('help text displays for connected calendars', async ({ page }) => {
    ...
    if (await connectedBadge.count() > 0) {
      ...
      await expect(page.locator('text=CalMill will automatically check for conflicts')).toBeVisible();
      await expect(page.locator('text=Events will be created in your primary calendar')).toBeVisible();
    }
  });
  ...
  test('future integrations section displays', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Other Calendar Providers', exact: true })).toBeVisible();
  ...
  test('error states display properly', async ({ page }) => {
    ...
    const errorAlert = page.locator('[role="alert"], .alert-destructive, .bg-destructive');
    ...
    if (await errorAlert.count() > 0) {
      ...
      await expect(page.locator('.alert-destructive .lucide-alert-circle, .alert-destructive [data-testid="error-icon"]')).toBeVisible();
    }
  });
  ...
  test('loading states work correctly', async ({ page }) => {
    ...
    const loadingSkeleton = page.locator('[data-testid="loading-skeleton"], .skeleton');
    const loadedContent = page.getByRole('heading', { name: 'Calendar Connections', exact: true });
  ...
  test('calendar color indicators display correctly', async ({ page }) => {
    ...
    if (await connectedBadge.count() > 0) {
      ...
      const colorIndicators = page.locator('.w-4.h-4.rounded, [data-testid="calendar-color"]');
