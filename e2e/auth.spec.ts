...
test.describe('Authentication', () => {
  ...
  test('signup with invalid email shows validation error', async ({ page }) => {
    ...
    const stillOnSignup =
      page.url().includes('/signup') || (await page.locator("input[type='email']").count()) > 0;
  ...
  test.fixme('demo login button auto-fills credentials', async ({ page }) => {
    await page.goto('/login');
  ...
  test('redirects to event-types page after login by default', async ({ page }) => {
    await loginAsDemoUser(page);
  ...
  test.fixme('unauthenticated access to dashboard redirects to login', async ({ page }) => {
    ...
    const isRedirectedToLogin =
      url.includes('/login') || (await page.locator('input[name="email"]').count()) > 0;
    expect(isRedirectedToLogin).toBeTruthy();
  });

  test('unauthenticated access to event types redirects to login', async ({ page }) => {
