...
test.describe("Availability Schedule Editor", () => {
  ...
  test.describe("Schedule overview", () => {
    ...
    test("shows all standard weekdays in the grid", async ({ page }) => {
      const days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"];
      for (const day of days) {
        await expect(
          page.locator(`text=${day}`).or(page.locator(`text=${day.slice(0, 3)}`)).first()
  ...
  test.describe("Time range editing", () => {
    ...
    test("time inputs are visible for enabled days", async ({ page }) => {
      ...
      if ((await timeInputs.count()) > 0) {
        ...
        const timeSelects = page
          .locator("select")
          .filter({ hasText: /AM|PM|09|17/ });
  ...
  test.describe("Date overrides", () => {
    ...
    test("date overrides section is visible", async ({ page }) => {
      await expect(
        page
          .locator("text=Date Override")
          .or(page.locator("text=date override"))
          .first()
      ).toBeVisible({ timeout: 10_000 });
    });
    ...
    test("clicking Add Date Override opens a dialog or date picker", async ({
      page,
    }) => {
      const addButton = page.locator("button", {
        hasText: "Add Date Override",
      ...
      const datePicker = page.locator(
        'input[type="date"], [aria-label*="date" i]'
      );
    ...
    test("seed creates date overrides that appear in the list", async ({
      page,
    }) => {
      // Seed creates date overrides (blocked day and modified hours)
      // They should appear in the date overrides section
      const overridesList = page
        .locator("text=Unavailable")
        .or(page.locator("text=unavailable"))
        .or(page.locator('[class*="override"]'))
        .first();

      // This may or may not be visible depending on UI rendering
  ...
  test.describe("Schedule timezone", () => {
    ...
    test("timezone information is displayed on the schedule", async ({
      page,
    }) => {
  ...
  test.describe("Multiple schedules", () => {
    ...
    test("can see multiple schedules if they exist", async ({ page }) => {
      ...
      await expect(
        page
          .locator("text=Business Hours")
          .or(page.locator("text=Extended Hours"))
          .first()
      ).toBeVisible({ timeout: 10_000 });
    });
  });
  ...
  test.describe("Weekend configuration", () => {
    ...
    test("Saturday and Sunday are shown in the weekly grid", async ({
      page,
    }) => {
      await expect(
        page
          .locator("text=Saturday")
          .or(page.locator("text=Sat"))
          .first()
      ).toBeVisible({ timeout: 10_000 });
      await expect(
        page
          .locator("text=Sunday")
          .or(page.locator("text=Sun"))
          .first()
      ).toBeVisible({ timeout: 10_000 });
    });
  });
