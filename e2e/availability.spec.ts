...
test.describe("Availability Management", () => {
  ...
  test.describe("Authenticated Availability", () => {
    authTest("loads availability page with correct content", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      await expect(page.locator("text=Configure your weekly schedule and date-specific overrides")).toBeVisible();
    });
    ...
    authTest("displays default schedule badge", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      const businessHoursCard = page.locator('[data-testid="schedule-card"]:has(text="Business Hours")');
      await expect(businessHoursCard.locator('text="Default"')).toBeVisible();
    });

    authTest("can view schedule availability grid", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      await page.locator('[data-testid="schedule-card"]:has(text="Business Hours")').click();
    ...
    authTest("can toggle day availability", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      await expect(mondayToggle).toHaveAttribute('aria-checked', String(!initialState));

      // Verify time inputs are shown/hidden accordingly
      if (!initialState) {
        // If we enabled the day, time inputs should be visible
        await expect(page.locator('[data-testid="start-time-1"]')).toBeVisible();
        await expect(page.locator('[data-testid="end-time-1"]')).toBeVisible();
      } else {
        // If we disabled the day, time inputs should be hidden
        await expect(page.locator('[data-testid="start-time-1"]')).not.toBeVisible();
        await expect(page.locator('[data-testid="end-time-1"]')).not.toBeVisible();
      }
    });
    ...
    authTest("can modify time ranges", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator('text="Schedule updated successfully"')).toBeVisible({ timeout: 5000 });
    });

    authTest("can view and add date overrides", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      const addOverrideButton = page.locator('button:has(text="Add Date Override")');
      ...
      const unavailableCheckbox = page.locator('input[type="checkbox"]:near(text="Unavailable")').last();
      await unavailableCheckbox.check();

      // Save the override
      const saveOverrideButton = page.locator('button:has(text="Save Override")');
    ...
    authTest("can delete date overrides", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      const deleteButtons = page.locator('[data-testid="delete-override-button"]');
    ...
    authTest("can create new schedule", async ({ authenticatedPage: page }) => {
      ...
      await expect(page.locator('text="Test Schedule"')).toBeVisible({ timeout: 5000 });
    });

    authTest("can switch between schedules", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
      ...
      const firstScheduleName = await page.locator('[data-testid="selected-schedule-name"]').textContent();
      ...
      const secondScheduleName = await page.locator('[data-testid="selected-schedule-name"]').textContent();
      expect(firstScheduleName).not.toBe(secondScheduleName);
    });

    authTest("validates time range constraints", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
    ...
    authTest("shows timezone information", async ({ authenticatedPage: page }) => {
      await page.goto("/availability");
