...
function SlotSkeleton() {
  return <div className="h-10 w-full animate-pulse rounded-md bg-gray-100" aria-hidden="true" />;
}
...
export function SlotList({
  ...
  if (slots.length === 0) {
    return (
      <div className={cn('flex flex-col items-center justify-center py-10 text-center', className)}>
        <p className="text-sm font-medium text-gray-500">No available times on this date</p>
        <p className="mt-1 text-xs text-gray-400">Please select another date</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {slots.map((slot) => {
        const isSelected = selectedSlot !== null && selectedSlot.time === slot.time;
