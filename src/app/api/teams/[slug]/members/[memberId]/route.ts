    ...
    if (data.role === "OWNER" || targetMember.role === "OWNER") {
      if (userMembership.role !== "OWNER") {
        return NextResponse.json(
          {
            error: "forbidden",
            message: "Only team owners can change ownership roles",
          },
          { status: 403 },
        );
      }
        ...
        return NextResponse.json(
          {
            error: "conflict",
            message:
              "Cannot demote the only team owner. Promote another member to owner first.",
          },
          { status: 409 },
        );
      }
        ...
        return NextResponse.json(
          {
            error: "conflict",
            message:
              "Cannot remove the only team owner. Promote another member to owner first.",
          },
          { status: 409 },
        );
      }
