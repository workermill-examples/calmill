...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { newStartTime, attendeeTimezone, reason } =
    ...
    if (
      ...
      originalBooking.status === "REJECTED"
    ) {
      return NextResponse.json(
        {
          error: "invalid",
    ...
    if (originalBooking.status === "RESCHEDULED") {
      return NextResponse.json(
        {
          error: "invalid",
          message: "This booking has already been rescheduled",
        },
        { status: 400 },
      );
    }
    ...
    const newEnd = new Date(
      newStart.getTime() + originalBooking.eventType.duration * 60 * 1000,
    );
    ...
    const requestedSlot = availableSlots.find(
      (slot) => new Date(slot.time).getTime() === newStart.getTime(),
    );
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const newBooking = await tx.booking.create({
        data: {
          ...
          startTime: newStart,
          endTime: newEnd,
          status: originalBooking.eventType.requiresConfirmation
            ? "PENDING"
            : "ACCEPTED",
    ...
    await triggerWebhooks(
      ...
      WEBHOOK_EVENTS.BOOKING_RESCHEDULED,
      {
