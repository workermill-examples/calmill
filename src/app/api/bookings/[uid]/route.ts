...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (booking.status === 'CANCELLED' || booking.status === 'REJECTED') {
      return NextResponse.json(
        { error: 'invalid', message: 'Booking is already cancelled or rejected' },
        { status: 400 }
      )
    }
    ...
    if (action === 'accept' || action === 'reject') {
      ...
      if (!session?.user || session.user.id !== booking.userId) {
        return NextResponse.json(
          { error: 'unauthorized', message: 'Only the host can accept or reject bookings' },
          { status: 401 }
        )
      }
      ...
      if (booking.status !== 'PENDING') {
        return NextResponse.json(
          { error: 'invalid', message: 'Only pending bookings can be accepted or rejected' },
          { status: 400 }
        )
      }
    }
    ...
    if (action === 'cancel') {
      ...
      if (!isHost) {
        ...
        return NextResponse.json(
          { error: 'unauthorized', message: 'Authentication required to cancel booking' },
          { status: 401 }
        )
      }
      ...
      if (booking.status !== 'ACCEPTED' && booking.status !== 'PENDING') {
        return NextResponse.json(
          { error: 'invalid', message: 'Only accepted or pending bookings can be cancelled' },
          { status: 400 }
        )
      }
    }
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'validation', message: 'Invalid update data', details: error.issues },
        { status: 400 }
      )
    }
