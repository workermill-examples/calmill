...
const listBookingsSchema = z.object({
  status: z.enum(['PENDING', 'ACCEPTED', 'CANCELLED', 'REJECTED', 'RESCHEDULED']).optional(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { status, eventTypeId, startDate, endDate, attendeeEmail, page = 1, limit = 20 } =
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'validation', message: 'Invalid query parameters', details: error.issues },
        { status: 400 }
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requestedSlot = availableSlots.find(slot =>
      new Date(slot.time).getTime() === startTime.getTime()
    )

    if (!requestedSlot) {
      return NextResponse.json(
        { error: 'conflict', message: 'Selected time slot is no longer available' },
        { status: 409 }
      )
    }
    ...
    let bookingsToCreate = [{
      startTime,
      endTime,
      recurringEventId: data.recurringCount && data.recurringCount > 1 ? crypto.randomUUID() : null,
    }]

    if (data.recurringCount && data.recurringCount > 1 && eventType.recurringEnabled) {
      ...
      for (let i = 0; i < data.recurringCount; i++) {
        ...
        if (eventType.recurringFrequency === 'weekly') {
          ...
          nextStart = new Date(startTime.getTime() + (i * 14 * 24 * 60 * 60 * 1000))
        ...
        const nextEnd = new Date(nextStart.getTime() + (eventType.duration * 60 * 1000))
        ...
        const nextSlot = nextAvailableSlots.find(slot =>
          new Date(slot.time).getTime() === nextStart.getTime()
        )

        if (!nextSlot) {
          return NextResponse.json(
            { error: 'conflict', message: `Recurring slot ${i + 1} is not available` },
            { status: 409 }
          )
        }
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'validation', message: 'Invalid booking data', details: error.issues },
        { status: 400 }
      )
    }
