...
export async function POST(request: Request) {
  try {
    ...
    const isRecurring =
      recurringCount > 1 && eventType.recurringEnabled && !!eventType.recurringFrequency;
    ...
    void sendBookingEmails(booking, eventType, status, bookingUser, extraNotificationEmails).catch(
      (err) => {
        console.error('[Bookings] Email notification failed:', err);
      }
    );
 ...
 */
function computeRecurringOccurrences(baseTime: Date, frequency: string, count: number): Date[] {
...
async function sendBookingEmails(
  ...
  const startTimeForAttendee = formatDateInTimezone(
    booking.startTime,
    attendeeTimezone,
    DATE_FORMAT
  );
