...
export async function GET(request: Request) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      `${errorRedirect}${encodeURIComponent(error)}`
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(
      `${errorRedirect}${encodeURIComponent("missing_params")}`
    );
  }
  ...
  if (!user) {
    return NextResponse.redirect(
      `${errorRedirect}${encodeURIComponent("user_not_found")}`
    );
  }
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(
      `${errorRedirect}${encodeURIComponent("oauth_not_configured")}`
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      `${errorRedirect}${encodeURIComponent("userinfo_failed")}`
    );
  }
  ...
  return NextResponse.redirect(
    `${appUrl}/settings/calendars?connected=true`
  );
}
