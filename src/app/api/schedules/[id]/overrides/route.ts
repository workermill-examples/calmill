...
const createDateOverrideSchema = z.object({
  ...
  {
    message: 'For available overrides, both startTime and endTime are required. For unavailable overrides, both should be omitted.',
  }
).refine(
  (data) => {
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(
      { error: 'internal_server_error', message: 'Failed to fetch date overrides' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (existingOverride) {
      return NextResponse.json(
        { error: 'conflict', message: 'Date override already exists for this date' },
        { status: 409 }
      )
    }
    ...
    if (overrideDate < today) {
      return NextResponse.json(
        { error: 'validation_error', message: 'Cannot create override for past dates' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      { error: 'internal_server_error', message: 'Failed to create date override' },
      { status: 500 }
    )
  }
}