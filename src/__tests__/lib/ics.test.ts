...
describe('ICS Library', () => {
  ...
  describe('generateICS', () => {
    ...
    it('should include all optional fields when provided', () => {
      ...
      expect(ics).toContain('ORGANIZER;CN=Jane Organizer:mailto:jane@example.com');
      expect(ics).toContain('ATTENDEE;CN=John Attendee;ROLE=REQ-PARTICIPANT:mailto:john@example.com');
    ...
    it('should escape special characters in text fields', () => {
      ...
      expect(ics).toContain('SUMMARY:Meeting with\\; commas\\, and \\\\backslashes\\nand newlines');
      expect(ics).toContain('DESCRIPTION:Description with\\; special\\, chars\\\\and\\nnewlines');
      expect(ics).toContain('LOCATION:Room\\; 123\\, Building\\\\A\\nFloor 2');
      expect(ics).toContain('ORGANIZER;CN=User\\; with\\, special\\\\chars\\ninname:mailto:user@example.com');
    });
    ...
    it('should handle multiple attendees', () => {
      ...
      expect(ics).toContain('ATTENDEE;CN=Alice;ROLE=REQ-PARTICIPANT:mailto:alice@example.com');
      expect(ics).toContain('ATTENDEE;CN=Bob;ROLE=REQ-PARTICIPANT:mailto:bob@example.com');
      expect(ics).toContain('ATTENDEE;CN=Charlie;ROLE=REQ-PARTICIPANT:mailto:charlie@example.com');
    });
    ...
    it('should handle different status values', () => {
      const statuses: Array<ICSEvent['status']> = ['TENTATIVE', 'CONFIRMED', 'CANCELLED'];
  ...
  describe('createBookingICS', () => {
    ...
    it('should create ICS from booking data with all fields', () => {
      ...
      expect(ics).toContain('ORGANIZER;CN=Jane Consultant:mailto:jane.consultant@example.com');
      expect(ics).toContain('ATTENDEE;CN=John Client;ROLE=REQ-PARTICIPANT:mailto:john.client@example.com');
    ...
    it('should handle booking without optional fields', () => {
      ...
      expect(ics).toContain('ATTENDEE;CN=Attendee;ROLE=REQ-PARTICIPANT:mailto:attendee@example.com');
    ...
    it('should not duplicate location in description when same as meeting URL', () => {
      ...
      const urlOccurrences = (description.match(/https:\/\/zoom\.us\/j\/123456789/g) || []).length;
  ...
  describe('createICSHeaders', () => {
    it('should create proper headers for ICS file download', () => {
      ...
      expect(headers).toEqual({
        ...
        'Content-Disposition': 'attachment; filename="test-event-2024-01-15.ics"',
    ...
    it('should handle filenames with special characters', () => {
      ...
      expect(headers['Content-Disposition']).toBe('attachment; filename="meeting (updated).ics"');
    });
  });
  ...
  describe('ICS compliance and edge cases', () => {
    ...
    it('should handle very long descriptions with proper folding', () => {
      ...
      const lines = ics.split('\r\n');
      const descriptionStartIndex = lines.findIndex(line => line.startsWith('DESCRIPTION:'));
