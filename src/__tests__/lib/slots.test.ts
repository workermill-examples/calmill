...
describe("getAvailableSlots", () => {
  ...
  it("falls back to user default schedule when event has no schedule", async () => {
    const eventTypeNoSchedule = {
      ...mockEventType,
      schedule: null,
      scheduleId: null,
    };
    vi.mocked(prisma.eventType.findUnique).mockResolvedValue(
      eventTypeNoSchedule as any,
    );

    // Mock fallback to default schedule
    vi.mocked(prisma.schedule.findFirst).mockResolvedValueOnce(
      mockSchedule as any,
    ); // For default schedule lookup
  ...
  it("integrates Google Calendar busy times", async () => {
    const eventTypeWithCalendar = {
      ...
      user: {
        id: "user-1",
        calendarConnections: [{ id: "connection-1", isPrimary: true }],
      },
    };

    vi.mocked(prisma.eventType.findUnique).mockResolvedValue(
      eventTypeWithCalendar as any,
    );
    vi.mocked(prisma.booking.findMany).mockResolvedValue([]);
  ...
  it("applies daily booking limits", async () => {
    ...
    vi.mocked(prisma.eventType.findUnique).mockResolvedValue(
      eventTypeWithDailyLimit as any,
    );
    vi.mocked(prisma.booking.findMany).mockResolvedValue([]);
  ...
  it("applies weekly booking limits", async () => {
    const eventTypeWithWeeklyLimit = {
      ...mockEventType,
      maxBookingsPerWeek: 5,
    };
    vi.mocked(prisma.eventType.findUnique).mockResolvedValue(
      eventTypeWithWeeklyLimit as any,
    );
    vi.mocked(prisma.booking.findMany).mockResolvedValue([]);
  ...
  it("handles errors gracefully", async () => {
    vi.mocked(prisma.eventType.findUnique).mockRejectedValue(
