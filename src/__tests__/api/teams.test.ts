...
import { GET as getTeam, PUT as updateTeam, DELETE as deleteTeam } from '@/app/api/teams/[slug]/route'
import { GET as getMembers, POST as inviteMember } from '@/app/api/teams/[slug]/members/route'
import { PUT as updateMember, DELETE as removeMember } from '@/app/api/teams/[slug]/members/[memberId]/route'
...
describe('Teams API', () => {
  ...
  describe('GET /api/teams', () => {
    ...
    it('should return user teams successfully', async () => {
      mockAuth.mockResolvedValue({ user: { id: 'user1', email: 'test@example.com' } })
          ...
          members: [
            {
              ...
              accepted: true,
              userId: 'user1',
              user: { id: 'user1', name: 'Test User', email: 'test@example.com', username: 'testuser' }
            }
          ],
          eventTypes: [
            { id: 'et1', title: 'Meeting', slug: 'meeting', duration: 30, isActive: true, schedulingType: null }
          ],
  ...
  describe('DELETE /api/teams/[slug]', () => {
    it('should require owner permissions', async () => {
      ...
      const response = await deleteTeam(
        new NextRequest('http://localhost:3000/api/teams/test-team', { method: 'DELETE' }),
        { params: Promise.resolve({ slug: 'test-team' }) }
      )
      const data = await response.json()
      ...
      mockPrisma.teamMember.findMany.mockResolvedValue([
        {
          ...
          accepted: true,
          createdAt: new Date(),
          user: { id: 'user1', name: 'Owner', email: 'owner@example.com', username: 'owner' }
        }
      ])
  ...
  describe('POST /api/teams/[slug]/members', () => {
    ...
    it('should invite member successfully', async () => {
      ...
      mockPrisma.teamMember.create.mockResolvedValue({
        ...
        accepted: false,
        createdAt: new Date(),
        user: { id: 'user2', name: 'New User', email: 'new@example.com', username: 'newuser' }
      })
  ...
  describe('GET /api/teams/invitations', () => {
    it('should require authentication', async () => {
      ...
      const response = await getInvitations(new NextRequest('http://localhost:3000/api/teams/invitations'))
      const data = await response.json()
  ...
  describe('POST /api/teams/invitations/[memberId]/accept', () => {
    it('should accept invitation successfully', async () => {
      ...
      const response = await acceptInvitation(
        new NextRequest('http://localhost:3000/api/teams/invitations/invite1/accept', { method: 'POST' }),
    ...
    it('should handle non-existent invitation', async () => {
      ...
      const response = await acceptInvitation(
        new NextRequest('http://localhost:3000/api/teams/invitations/invalid/accept', { method: 'POST' }),
  ...
  describe('POST /api/teams/invitations/[memberId]/reject', () => {
    it('should reject invitation successfully', async () => {
      ...
      const response = await rejectInvitation(
        new NextRequest('http://localhost:3000/api/teams/invitations/invite1/reject', { method: 'POST' }),
        { params: Promise.resolve({ memberId: 'invite1' }) }
      )
      const data = await response.json()
        ...
        members: [
          {
            id: 'member1',
            role: 'OWNER',
            user: { id: 'user1', name: 'Owner', username: 'owner', image: null }
          }
        ],
        eventTypes: [
          {
