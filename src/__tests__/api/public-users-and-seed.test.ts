...
describe('Public User Routes and Seed Endpoint', () => {
  ...
  describe('GET /api/users/[username] - Public User Profile', () => {
    ...
    it('should handle special characters in username', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/users/${specialUsername}`)
    ...
    it('should not expose sensitive user data', async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValue(userWithSensitiveData as any)
  ...
  describe('GET /api/users/[username]/event-types - Public User Event Types', () => {
    it('should return event types for existing user', async () => {
      ...
      vi.mocked(prisma.eventType.findMany).mockResolvedValue([mockEventType] as any)

      const request = new NextRequest('http://localhost:3000/api/users/testuser/event-types')
      const params = Promise.resolve({ username: 'testuser' })
    ...
    it('should return 404 for non-existent user', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/users/nonexistent/event-types')
      const params = Promise.resolve({ username: 'nonexistent' })
      const response = await getPublicUserEventTypes(request, { params })
      const data = await response.json()
    ...
    it('should parse custom questions from JSON string', async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValue({ id: 'user-123' } as any)
      vi.mocked(prisma.eventType.findMany).mockResolvedValue([eventTypeWithCustomQuestions] as any)
    ...
    it('should handle null custom questions', async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValue({ id: 'user-123' } as any)
      vi.mocked(prisma.eventType.findMany).mockResolvedValue([eventTypeWithNullCustomQuestions] as any)
    ...
    it('should handle database errors gracefully', async () => {
      vi.mocked(prisma.user.findUnique).mockResolvedValue({ id: 'user-123' } as any)
      vi.mocked(prisma.eventType.findMany).mockRejectedValue(new Error('Database error'))
    ...
    it('should include all required fields in response', async () => {
      ...
      const expectedFields = [
        'id', 'title', 'slug', 'description', 'duration', 'requiresConfirmation',
        'price', 'currency', 'minimumNotice', 'beforeBuffer', 'afterBuffer',
        'slotInterval', 'maxBookingsPerDay', 'maxBookingsPerWeek', 'futureLimit',
        'color', 'customQuestions', 'recurringEnabled', 'recurringFrequency',
        'createdAt', 'schedule', 'bookingCount'
      ]
  ...
  describe('POST /api/seed - Seed Database', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(prisma.user.upsert).mockResolvedValue({ id: 'demo-user-123' } as any)
      vi.mocked(prisma.schedule.upsert).mockResolvedValue({ id: 'schedule-123' } as any)
      vi.mocked(prisma.eventType.upsert).mockResolvedValue({ id: 'event-type-123' } as any)
      vi.mocked(prisma.availability.upsert).mockResolvedValue({ id: 'availability-123' } as any)
      vi.mocked(prisma.booking.upsert).mockResolvedValue({ id: 'booking-123' } as any)
      vi.mocked(prisma.dateOverride.upsert).mockResolvedValue({ id: 'override-123' } as any)
      vi.mocked(prisma.team.upsert).mockResolvedValue({ id: 'team-123' } as any)
      vi.mocked(prisma.teamMember.upsert).mockResolvedValue({ id: 'member-123' } as any)
      ...
      vi.mocked(prisma.schedule.create).mockResolvedValue({ id: 'schedule-123' } as any)
      vi.mocked(prisma.availability.create).mockResolvedValue({ id: 'availability-123' } as any)
      vi.mocked(prisma.dateOverride.create).mockResolvedValue({ id: 'override-123' } as any)
      vi.mocked(prisma.eventType.create).mockResolvedValue({ id: 'event-type-123' } as any)
      vi.mocked(prisma.booking.create).mockResolvedValue({ id: 'booking-123' } as any)
      vi.mocked(prisma.team.create).mockResolvedValue({ id: 'team-123' } as any)
      vi.mocked(prisma.teamMember.create).mockResolvedValue({ id: 'member-123' } as any)
    ...
    it('should be idempotent - safe to run multiple times', async () => {
      ...
      expect(vi.mocked(prisma.user.upsert).mock.calls.length).toBeGreaterThan(0)
    ...
    it('should create event types with proper JSON custom questions', async () => {
      ...
      const callsWithQuestions = eventTypeCalls.filter(call =>
        call[0].create.customQuestions && call[0].create.customQuestions !== 'undefined'
      )
