...
import { GET as getEventType, PUT, DELETE } from '@/app/api/event-types/[id]/route'
...
describe('Event Types API', () => {
  ...
  describe('GET /api/event-types', () => {
    ...
    it('should include inactive event types when requested', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/event-types?includeInactive=true')
      const response = await GET(request)
  ...
  describe('GET /api/event-types/[id]', () => {
    it('should require authentication', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/event-types/et1')
      const response = await getEventType(request, { params: Promise.resolve({ id: 'et1' }) })
      const data = await response.json()
    ...
    it('should return 404 for non-existent event type', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/event-types/nonexistent')
      const response = await getEventType(request, { params: Promise.resolve({ id: 'nonexistent' }) })
      const data = await response.json()
  ...
  describe('PATCH /api/event-types/[id]/toggle', () => {
    it('should require authentication', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/event-types/et1/toggle', { method: 'PATCH' })
      const response = await PATCH(request, { params: Promise.resolve({ id: 'et1' }) })
      const data = await response.json()
  ...
  describe('DELETE /api/event-types/[id]', () => {
    it('should require authentication', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/event-types/et1', { method: 'DELETE' })
      const response = await DELETE(request, { params: Promise.resolve({ id: 'et1' }) })
      const data = await response.json()
    ...
    it('should delete event type without active bookings', async () => {
      ...
      expect(mockPrisma.eventType.delete).toHaveBeenCalledWith({ where: { id: 'et1' } })
    })
