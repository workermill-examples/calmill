...
describe("Schedules API", () => {
      ...
      mockPrisma.schedule.findMany.mockResolvedValue([
        {
          ...
          isDefault: true,
          availabilities: [{ day: 1, startTime: "09:00", endTime: "17:00" }],
          dateOverrides: [],
          _count: { eventTypes: 2 },
        },
        {
          ...
          timezone: "America/New_York",
          isDefault: false,
          availabilities: [{ day: 1, startTime: "08:00", endTime: "20:00" }],
          dateOverrides: [],
          _count: { eventTypes: 0 },
        },
      ]);
  ...
  describe("GET /api/schedules/[id]", () => {
    it("should require authentication", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/schedules/sched1",
      );
      const response = await getSchedule(request, {
        params: Promise.resolve({ id: "sched1" }),
      });
      const data = await response.json();
    ...
    it("should return 404 for non-existent schedule", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/schedules/nonexistent",
      );
      const response = await getSchedule(request, {
  ...
  describe("GET /api/schedules/[id]/overrides", () => {
    it("should require authentication", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/schedules/sched1/overrides",
      );
      const response = await getOverrides(request, {
        params: Promise.resolve({ id: "sched1" }),
      });
      const data = await response.json();
    ...
    it("should filter by date range when provided", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/schedules/sched1/overrides?startDate=2024-12-01&endDate=2026-12-31",
      );
      const response = await getOverrides(request, {
        params: Promise.resolve({ id: "sched1" }),
      });
  ...
  describe("POST /api/schedules/[id]/overrides", () => {
    it("should require authentication", async () => {
      ...
      const request = new NextRequest(
        ...
        {
          method: "POST",
          body: JSON.stringify({ date: "2026-12-25", isUnavailable: true }),
        },
      );
      const response = await createOverride(request, {
        params: Promise.resolve({ id: "sched1" }),
      });
      const data = await response.json();
  ...
  describe("DELETE /api/schedules/[id]/overrides/[overrideId]", () => {
    it("should require authentication", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/schedules/sched1/overrides/override1",
        { method: "DELETE" },
      );
    ...
    it("should delete date override", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.message).toContain("deleted successfully");
      expect(mockPrisma.dateOverride.delete).toHaveBeenCalledWith({
        where: { id: "override1" },
      });
    });
    ...
    it("should return 404 for non-existent override", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/schedules/sched1/overrides/nonexistent",
        { method: "DELETE" },
      );
      const response = await deleteOverride(request, {
