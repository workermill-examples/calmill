...
describe('API Routes', () => {
  ...
  describe('Slots Endpoint', () => {
    ...
    it('should handle slot calculation errors gracefully', async () => {
      vi.mocked(getAvailableSlots).mockRejectedValue(new Error('Slot calculation failed'))
  ...
  describe('Authentication Middleware Behavior', () => {
    ...
    it('should validate user ID format', () => {
      ...
      const invalidUserIds = ['', '  ', null, undefined, 'user@example.com']
    ...
    it('should handle database constraint errors', () => {
      ...
      const handleError = (error: Error) => {
        if (error.name === 'PrismaClientKnownRequestError') {
          return { status: 409, error: 'conflict', message: 'Resource already exists' }
        }
  ...
  describe('Pagination and Filtering', () => {
    ...
    it('should handle status filters', () => {
      ...
      const filterStatus = (status: string | null) => {
        ...
        return validStatuses.includes(status.toUpperCase()) ? status.toUpperCase() : null
      }
  ...
  describe('Response Formatting', () => {
    ...
    it('should format error responses consistently', () => {
      ...
      const response = formatError('validation_error', 'Invalid input', { field: 'email' })
    ...
    it('should handle list responses with metadata', () => {
      const formatListResponse = (items: any[], total: number, limit: number, offset: number) => {
        return {
          data: items,
          meta: {
            total,
